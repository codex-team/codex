...
class Model_Methods extends Model
{
    ...
    */
    private static function countEstimateReadingTime( $str ){
        ...
        // $str = str_word_count(strip_tags($str));
        // $time = ceil($str / $WordsPerMinute);
        // return $time;
        //

        $speed = 1050;
        $justText = str_replace(' ', '', strip_tags($str));
        $justText = str_replace(array('_', '-', '—'), '', $justText);
        return min(100, max(1, round(ceil(mb_strlen($justText)/1000)*1000/$speed)));
